...
from feature_engineering.regression_metrics import REGRESSION_METRICS
from feature_engineering.unary_features import ALL_UNARY_FEATURES, NN_UNARY_FEATURES, unary_feature_wrapper
from feature_engineering.binary_features import ALL_BINARY_FEATURES, NN_BINARY_FEATURES, binary_feature_wrapper


REGRESSION_MACHINES = (
    Ridge(),
    LinearRegression(),
    DecisionTreeRegressor(random_state=0, min_samples_leaf=3),
    RandomForestRegressor(random_state=0, n_estimators=10, min_samples_leaf=3),
...
def regression_features(clf, binarize_x=False):
    def inner_func(x, y):
        ...
        for f in (ALL_UNARY_FEATURES + NN_UNARY_FEATURES)[:1]:
